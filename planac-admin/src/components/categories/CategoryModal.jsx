...
export default function CategoryModal({ category, categories, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (category) {
        // Editar
        await axios.put(`${API_URL}/api/admin/categories/${category.id}`, formData);
      } else {
        // Criar
  ...
  // Filtrar categorias disponíveis para categoria pai (não pode ser ela mesma ou suas filhas)
  const availableParentCategories = categories.filter(cat => {
    if (category && cat.id === category.id) return false;
    if (category && cat.categoria_pai_id === category.id) return false;
    return true;
  });
            ...
            <div>
              ...
              >
                ...
                {availableParentCategories
                  .filter(cat => !cat.categoria_pai_id) // Apenas categorias raiz
                  .map(cat => (

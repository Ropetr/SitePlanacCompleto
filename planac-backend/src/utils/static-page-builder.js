/**
 * ===========================================
 * STATIC PAGE BUILDER - Geração de Arquivos HTML Estáticos
 * ===========================================
 * Gera arquivos .html físicos e faz commit/deploy automático
 */

import { generatePageHTML } from './page-builder.js';

/**
 * Template de página usando o template atual do site
 */
...
/**
 * Gera arquivo HTML estático e faz commit/deploy
 */
export async function buildAndDeployPage(page, env, githubToken) {
  try {
    console.log(`🔨 Gerando HTML estático para página: ${page.nome}`);
    ...
    const repoOwner = 'Ropetr'; // Ajustar conforme seu usuário GitHub
    const repoName = 'SitePlanacCompleto';
    const branch = 'main';

    // 3. Verificar se arquivo já existe no GitHub
    const fileCheckUrl = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/${fileName}?ref=${branch}`;
    ...
    try {
      ...
      if (checkResponse.ok) {
        ...
        console.log(`📄 Arquivo ${fileName} já existe - SHA: ${fileSha}`);
      }
    } catch (error) {
    ...
    const commitUrl = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/${fileName}`;

    const commitData = {
      message: `chore: Auto-rebuild de ${page.nome} (${page.slug})`,
      content: Buffer.from(html).toString('base64'),
      branch: branch
    };

    if (fileSha) {
      commitData.sha = fileSha; // Necessário para atualizar arquivo existente
    }
    ...
    if (!commitResponse.ok) {
      ...
      throw new Error(`Erro ao fazer commit: ${commitResponse.status} - ${errorText}`);
    }

    const commitResult = await commitResponse.json();
    console.log(`✅ Commit realizado: ${commitResult.commit.sha}`);

    // 5. Cloudflare Pages faz deploy automático após push no GitHub
    console.log(`🚀 Cloudflare Pages iniciará deploy automático em instantes...`);

    return {
      success: true,
      fileName,
      commitSha: commitResult.commit.sha,
      message: 'Página rebuiltada e commit realizado. Deploy automático iniciado.'
    };

  } catch (error) {
    console.error('❌ Erro ao build and deploy:', error);
    throw error;
  }
}

export { generateStaticPageHTML };
